...
{
	int ID;
	COOK_TYPE type;	// for each order type there is a corresponding type (VIP, Normal, Vegan)
	double speed;		// dishes it can prepare in one clock tick (in one timestep)

	COOK_STATUS status;  // --> avaialable, busy or on_break or in_rest
	...
	bool injuredThisTime;
	bool injuredBefore;     
	bool assignedInRest;  // should be used in  checkcooksInBreakAndRest
...
public:
	...
	void setSpeed ( int s );     // -->
	double getSpeed ( ) const;     // --> for finding FinishOrderOrBreakTime before moving to assigned queue
	...
	void setInjuredThisTime ( bool boolean );
	bool getInjuredThisTime ();

	void setInjuredBefore ( bool boolean );
	bool getInjuredBefore ();

	void setAssignedInRest ( bool boolean );
	bool getAssignedInRest ();
